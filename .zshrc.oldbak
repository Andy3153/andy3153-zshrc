 ...
 # Every theme separated into functions
   ...
   # Blocks theme
     function _blockstheme ()
     {
       #emojis=('☺' '☻' '🌜' '🐭' '🐮' '🐱' '🐵' '😀' '😁' '😂' '😃' '😄' '😅' '😆' '😇' '😈' '😉' '😊' '😋' '😌' '😍' '😎' '😏' '😗' '😘' '😙' '😚' '😛' '😜' '😝' '😴' '😵' '😶' '😷' '😸' '😹' '😺' '😻' '😼' '😽' '😾' '😿' '🙀' '🙃' '🤖')
       # ${emojis[$(($RANDOM%43))]}
       export PS1="%(?.%K{green}%F{black} √ %f%k.%K{red}%F{black} × %f%k)%K{black} %n@%M %k%K{$_promptcolor}%F{black} > %f%k "
   ...
   # KeepItSimple
     function _keepitsimple ()
     {
       export PS1="%F{$_promptcolor}\ %n@%m \ %(?.%F{green}.%F{red})> %f%F{$_promptcolor}\%f "
       export RPS1="%F{$_promptcolor} / %(5~|%-2~/…/%2~|%4~) /%f%F{yellow} %h %F{$_promptcolor}/"
     }
